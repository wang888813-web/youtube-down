...
export default function ZhProfilePage() {
  ...
  const tabs = [
    { id: "overview" as Tab, label: "概览", icon: BarChart2 },
    { id: "history" as Tab, label: "下载记录", icon: Clock },
    { id: "referrals" as Tab, label: "推荐计划", icon: Users },
  ];
      ...
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              {isUnlimited ? (
                ...
                <span className="flex items-center gap-1 text-xs bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 px-2.5 py-0.5 rounded-full font-semibold">
                  <Star className="w-3 h-3" /> Pro 会员
                </span>
              ) : (
                <span className="flex items-center gap-1 text-xs bg-gray-700 text-gray-400 px-2.5 py-0.5 rounded-full">
                  <Shield className="w-3 h-3" /> 免费版
                </span>
              )}
              {userData?.plan_expires_at && (
                <span className="text-xs text-gray-500">
                  到期 {new Date(userData.plan_expires_at * 1000).toLocaleDateString("zh-CN")}
                </span>
              )}
              {userData?.created_at && (
                <span className="text-xs text-gray-600">
                  注册于 {new Date(userData.created_at * 1000).toLocaleDateString("zh-CN")}
                </span>
              )}
            </div>
          </div>
          <button
            onClick={() => { logout(); router.push("/zh"); }}
          ...
          >
            ...
            <span className="hidden sm:block text-xs">退出</span>
          </button>
        </div>
          ...
          <>
            ...
            {!isPro && (
              <div className="bg-gradient-to-r from-red-600/20 to-orange-600/20 border border-red-500/20 rounded-3xl p-5 flex items-center justify-between gap-4">
                <div>
                  <p className="text-white font-semibold text-sm mb-0.5">升级 Pro 会员</p>
                  <p className="text-gray-400 text-xs">每月 500 次下载 · 4K 画质 · 去广告 · 仅 $9.9/月</p>
                </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6">
            ...
            {history.length === 0 ? (
              <div className="text-center py-10">
                ...
                <Link href="/zh" className="text-red-400 text-sm hover:text-red-300 mt-1 inline-block">去下载视频 →</Link>
              </div>
            ) : (
                  ...
                  <a key={item.id} href={item.video_url} target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        <span className="text-xs text-gray-600">{new Date(item.created_at * 1000).toLocaleDateString("zh-CN")}</span>
                      </div>
                    </div>
        ...
        {tab === "referrals" && (
          <>
            <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">你的专属推荐链接</h2>
              <p className="text-gray-500 text-xs mb-4">好友注册即送你 1 天 Pro · 好友付费后再送 15 天 Pro</p>
              {referralData ? (
                <>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="bg-gray-800/60 rounded-2xl p-3 text-center">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">奖励天数</p>
                    </div>
                  </div>
                </>
              ) : (
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6">
              ...
              <div className="space-y-3">
                {[
                  { icon: Users, color: "blue", title: "好友注册", desc: "好友通过你的链接注册，立即获得 1 天 Pro 体验" },
                  { icon: TrendingUp, color: "green", title: "好友付费", desc: "好友订阅任意付费套餐后，再额外奖励 15 天 Pro" },
                  { icon: Gift, color: "yellow", title: "无上限累积", desc: "推荐越多获得越多，Pro 天数叠加计算，永不清零" },
                  ...
                  {referralData.referrals.map(r => (
                    <div key={r.id} className="flex items-center gap-3 p-3 bg-gray-800/40 rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-500 text-xs">{new Date(r.created_at * 1000).toLocaleDateString("zh-CN")}</p>
                      </div>
