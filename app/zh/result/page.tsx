  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <p className="text-gray-400">AI 正在处理视频...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <a href="/zh/script" className="text-red-400 hover:text-red-300 text-sm">← 重新输入链接</a>
      </div>
    );
  }
      ...
      <div className="flex gap-1 bg-gray-900 rounded-xl p-1 mb-6 w-fit">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === tab.key ? "bg-red-600 text-white" : "text-gray-400 hover:text-white"}`}
          >
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === "summary" && (
        <div className="space-y-6">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
            <h2 className="font-semibold mb-3 text-gray-300 text-sm uppercase tracking-wide">AI 摘要</h2>
            <p className="text-gray-200 leading-relaxed">{result.summary}</p>
          </div>
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
            <h2 className="font-semibold mb-4 text-gray-300 text-sm uppercase tracking-wide">关键要点</h2>
      ...
      {activeTab === "transcript" && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-semibold text-gray-300 text-sm uppercase tracking-wide">完整字幕</h2>
...
export default function ZhResultPage() {
  return (
